...
public class Strings {
	...
	boolean isPal = true;
	String muta = string.toLowerCase().replaceAll("\\d+", "").replaceAll("\\s+", "").replaceAll("\\p{Punct}", ""); // substring(0
	// (muta.length()/2
	int j = 0;
	for (int i = muta.length() - 1; i >= (muta.length() / 2); i--) {

	    if (muta.charAt(i) != muta.charAt(j)) {
		isPal = false;
	    }
	    ;
	    j++;
...
//    public static boolean isPalindrome(final String string) {
//	String reverse = "";
//	String mod = string.toLowerCase().replaceAll("\\d+", "").replaceAll("\s+", "").replaceAll("\\p{Punct}", "");
//	for (int i = mod.length() - 1; i >= 0; i--) {
//
//	    reverse = reverse + mod.charAt(i);
//	}
//	if (mod.equals(reverse)) {
//
//	    return true;
//	}
//	return false;
//    }
	...
	boolean poo = true;
	String pee = "1234567890";
	String barf = "_";

	if (pee.contains(name.substring(0, 1))) {
	    poo = false;
	}

	if (barf.contains(name.substring(0, 1))) {
	    if (name.length() == 1) {
		poo = false;
	    }
	}
